...
export const SCP106SetPoint = {
  onUse: (arg: ItemComponentUseEvent) => {
    ...
    if (result.successCount > 0) {
      ...
      const LastPoint = world.getDynamicProperty("scp106telepoint") as Vector3;
...
export const SCP106GoToPoint = {
  onUse: (arg: ItemComponentUseEvent) => {
    ...
    const dimension = player.dimension;
    arg.itemStack?.getComponent(`cooldown`)?.startCooldown(player);
    const Point = world.getDynamicProperty("scp106telepoint") as Vector3;
...
export const MTF_Terminal = {
  onUse: (arg: ItemComponentUseEvent) => {
    ...
    const player = arg.source as any;
...
export const SCP066RushStick = {
  onUse: (arg: ItemComponentUseEvent) => {
    ...
    function SCP066Stick() {
      const TargetHealth = Target?.getComponent("minecraft:health")?.currentValue as number;
      if (StickNum > 0 && TargetHealth > 0) {
        ...
        const TargetViewDirection = Target?.getViewDirection() as Vector3;
        // 获取目标的当前位置
        const TargetPosition = Target?.location as Vector3; // Vector3 对象
...
export const ChangeModels = {
  onUse: (arg: ItemComponentUseEvent) => {
    const player = arg.source;
    const playerAny = arg.source as any;
    let ModalForm;
