import { Dimension, Entity, EntityType, GameMode, ItemStack, Player, Vector3, world } from "@minecraft/server";
...
export function FormatTimeTick(ticks: number) {
...
export function parseVector3(input: string | undefined | null): Vector3 | undefined {
...
export function ClassesName(player: Player | Entity) {
...
export function PlaySoundForAllPlayer(SoundID: string) {
...
// 移除所有标签
export function RemoveAllTags(player: Player) {
...
export function SpawnCorpseOnPlayerDeath() {
  world.afterEvents.entityDie.subscribe((arg) => {
    ...
    if (!deadEntity || typeof deadEntity.location !== "object") return;
    ...
    if (deadEntity.typeId === "minecraft:player") {
      ...
      if (!playerTags.includes("scp") || (playerTags.includes("scp035") && playerTags.includes("scp"))) {
        const cropse = dimension?.spawnEntity("ei:corpse" as any, location);
        if (cropse) {
