...
// 设置皮肤状态
export function SetSkinState(player: string, SkinID: string, State: string) {
...
// 获取皮肤状态
export function GetSkinState(player: Player, SkinID: string) {
...
export function LoadSkinInfo() {
  for (const player of world.getAllPlayers()) {
    for (const SkinID in SkinList) {
      const Skin_Name = `${SkinList[SkinID as keyof typeof SkinList]}|${player.name}`;
      const getstate = world.getDynamicProperty(Skin_Name);
      let state: string;
      if (getstate) {
        state = getstate as string;
      } //
      else {
        state = SkinStateString.false;
      }
      SkinMap.set(Skin_Name, state as string);
