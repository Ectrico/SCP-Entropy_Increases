...
import {
  ClassesName,
  ClearAllCorpse,
  PlaySoundForAllPlayer,
  ShowNameTagClasses,
  SpawnCorpseOnPlayerDeath,
} from "./misc";
import { MusicVote } from "./activity";
...
import { sprayCanComponentRegistry, SprayPlayerState } from "./items_function/spray_can";
...
// 模式修改函数
export function setGameMode(mode: typeof Entropy_Increases | typeof Inbreak) {
  GameMode = mode;
}
// 地图
...
//
// 倒计时函数
let ResetTimeIntervalId: number;
...
let RoundTimeIntervalId: number;
...
//
//
export let CustomSupportLimit: number | undefined;
// 对局事件
// 事件控制器
let PreTimeStart = 1;
...
system.runInterval(() => {
  if (PreTime === GameMode.PreTimeDef && PreTimeStart === 1) {
    ...
    if (GameMode === Entropy_Increases) {
      ...
      system.runTimeout(() => {
        world.sendMessage(
          `§e[ANNOUNCE] §cThe site is experiencing multiple Keter and Euclid-level containment breaches. Full-site lockdown initiated.`
        );
        // 角色生成
    ...
    if (GameMode === Infection) {
      ...
      system.runTimeout(() => {
        spawn008_1(
          world.getAllPlayers()[Math.floor(Math.random() * world.getAllPlayers().length)],
...
system.runInterval(() => {
  if (RoundTime === GameMode.RoundTimeDef && RoundTimeStart === 1) {
    ...
    if (GameMode === Infection) {
      world.sendMessage(
        `§e[ANNOUNCE]§c设施遭遇SCP-008病毒泄露，§e请设施内的所有D级人员与安保人员合作，§6杀死所有SCP-008实体`
      );
      world.sendMessage(
        `§e[ANNOUNCE]§c设施遭遇SCP-008病毒泄露，§e请设施内的所有D级人员与安保人员合作，§6杀死所有SCP-008实体`
      );
...
system.runInterval(() => {
  if (GameMode === Inbreak) {
    if (RoundTime === 660 && RoundTime660 === 1) {
      ...
      world.sendMessage(
        `§e[ANNOUNCE] §bControl to NTF. We are now checking the camera feeds for potential threats. You should see any unauthorized intruders or escapees pinged on your navigation devices.`
      );
      RoundTime660 = 0;
    }
  }
...
world.afterEvents.chatSend.subscribe((arg: ChatSendAfterEvent) => {
  ...
  if (
    (Dev.includes(senderID) || Operator) &&
  ...
  if ((Dev.includes(senderID) || Operator) && message.split(" ")[0] === "#exp") {
    ...
    if (message.split(" ")[1] === "get") {
      sender.sendMessage(
        `§e[GameMode]>_: §f玩家：${message.split(" ")[2]} 经验：${ExpGetExp(
          `${message.split(" ")[2]}`
      ...
      if (message.split(" ")[2] === "exp") {
        ...
        sender.sendMessage(
          `§e[GameMode]>_: §f已将玩家：${message.split(" ")[3]} 的经验点数设置为 ${message.split(" ")[4]}`
        );
        sender.sendMessage(
          `§e[GameMode]>_: §f玩家：${message.split(" ")[3]} 现有经验点数：${ExpGetExp(message.split(" ")[3])}`
        );
      } //
      ...
      if (message.split(" ")[2] === "exp") {
        ...
        sender.sendMessage(
          `§e[GameMode]>_: §f已为玩家：${message.split(" ")[3]} 添加经验点数 ${message.split(" ")[4]}`
        );
        sender.sendMessage(
          `§e[GameMode]>_: §f玩家：${message.split(" ")[3]} 现有经验点数：${ExpGetExp(
            message.split(" ")[3]
...
//
// 玩家加入事件
world.afterEvents.playerJoin.subscribe((arg: PlayerJoinAfterEvent) => {
...
// }, 100);
let DeadCount = 0;
world.afterEvents.entityDie.subscribe((arg: EntityDieAfterEvent) => {
  const DiedEntity = arg.deadEntity;
  if (DiedEntity.typeId === "minecraft:player") {
    const DiedPlayer = DiedEntity as Player;
    ...
    let DiedPlayerClasses: string | undefined;
...
//
// 玩家重生事件
world.afterEvents.playerSpawn.subscribe((arg: PlayerSpawnAfterEvent) => {
  const RespawnPlayer = arg.player;
  const RespawnPlayerID = arg.player.name as string;
...
export function EscapeRewardDrops(RandomPlayer: string) {
  Drops(RandomPlayer);
}

function Drops(player: string) {
  // 获取当前现实世界时间
  const CurrentTime = new Date();
  const DropProbability = Math.random();
  let RandomItem: String | undefined;
  function DropsMassage() {
    world.sendMessage(
      `§d[掉落奖励] §a恭喜 §b${player} §a获得 ${RandomItem} §a--- ${CurrentTime.getFullYear()}-${
