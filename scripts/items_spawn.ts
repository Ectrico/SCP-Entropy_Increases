import { world, system, Dimension, Vector3, BlockPermutation } from "@minecraft/server";
...
export function RandomItemSpawn() {
  ...
  for (const vector of RandomItemsPoint.SCP500_Or_SCP420J) {
    ...
    const Vector = vector as Vector3;
    if (Math.random() < 0.7) {
  ...
  for (const [type, vector] of RandomItemsPoint.Lv1) {
    ...
    if (Math.random() < 0.7) {
      if (type === "shelf" || type === "cabinet") {
        ...
        const NewState = Shelf?.permutation.withState("ft:contain", "1") as BlockPermutation;
  ...
  for (const [type, vector] of RandomItemsPoint.Lv2) {
    ...
    if (Math.random() < 0.8) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", "2") as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
  ...
  for (const [type, vector] of RandomItemsPoint.Lv3) {
    ...
    if (Math.random() < 0.7) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", "3") as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
  ...
  for (const [type, vector] of RandomItemsPoint.Lv4) {
    ...
    if (Math.random() < 0.6) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", "4") as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
  ...
  for (const [type, vector] of RandomItemsPoint.Lv5) {
    ...
    if (Math.random() < 0.5) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", "5") as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
  ...
  for (const [type, vector] of RandomItemsPoint.LvCI) {
    ...
    if (Math.random() < 0.3) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", "ci") as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
  ...
  for (const [type, vector] of RandomItemsPoint.LvDepartment) {
    ...
    if (Math.random() < 0.1) {
      if (type === "shelf" || type === "cabinet") {
        const Shelf = overworld.getBlock(Vector);
        const NewState = Shelf?.permutation.withState("ft:contain", RandomCard) as BlockPermutation;
        Shelf?.setPermutation(NewState);
      } //
      else if (type === "drops") {
...
export function ClearAllItems() {
  ...
  for (const vector of FixedItemsPoint.EmptyShelf) {
    const Vector = vector as Vector3;
    const Shelf = overworld.getBlock(Vector);
    const NewState = Shelf?.permutation.withState("ft:contain", "empty") as BlockPermutation;
    Shelf?.setPermutation(NewState);
  }
  // Empty_Cabinet
...
function RandonGunString(type: "pistol" | "smg" | "rifle"): string {
  if (type === "pistol") {
