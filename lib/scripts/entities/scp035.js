import { world, EquipmentSlot, ItemStack, ItemLockMode } from "@minecraft/server";
...
export function equipSCP035OnInteract() {
    world.afterEvents.playerInteractWithEntity.subscribe((arg) => {
        ...
        if (entity.typeId === "ei:scp035_entity" && !player.hasTag("scp")) {
            ...
            const headSlot = (_a = player.getComponent("minecraft:equippable")) === null || _a === void 0 ? void 0 : _a.getEquipmentSlot(EquipmentSlot.Head);
            headSlot === null || headSlot === void 0 ? void 0 : headSlot.setItem(scp035);
            ...
            for (let slot = SCP_035.inventory[0]; slot < SCP_035.inventory[1]; slot++) {
                dimension.runCommand(`replaceitem entity "${player.name}" slot.inventory ${slot} ei:disable 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
            }
            for (let slot = SCP_035.hotbar[0]; slot < SCP_035.hotbar[1]; slot++) {
                dimension.runCommand(`replaceitem entity "${player.name}" slot.hotbar ${slot} ei:disable 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
            }
            for (let i = 0; i < SCP_035.items.length; i++) {
                dimension.runCommand(`give "${player.name}" ${SCP_035.items[i][0]} ${SCP_035.items[i][1]} 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
            }
            for (let i = 0; i < SCP_035.armor.length; i++) {
                dimension.runCommand(`replaceitem entity "${player.name}" ${SCP_035.armor[i][0]} 0 ${SCP_035.armor[i][1]} 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
            }
        }
    });
...
//# sourceMappingURL=scp035.js.map