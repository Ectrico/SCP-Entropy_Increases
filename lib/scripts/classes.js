...
import { SpawnPointNTF, SpawnPointCI, SpawnPointGuard, SpawnPointSCP035, SpawnPointClassD, SpawnPointSCP173, SpawnPointSCP966, SpawnPointRes, SpawnPointSCP106, SpawnPointSH, SpawnPointSCP457, SpawnPointSNE, SpawnPointSCP066, SpawnPointSCP049, SpawnPointSCP096, SpawnPointGOC, } from "./maps/ft_site_15";
import { CustomSupportLimit } from "./main";
import { PlaySoundForAllPlayer } from "./misc";
import { SCP049ChooseType, SpawnPointSCP049_2 } from "./character_skills";
// 定义主世界维度
let overworld;
// 定义下界维度
let nether;
...
export const ClassD_Fugitive = {
    ...
    subtitle: "§6D级人员 - 潜逃者",
    target_tip: "§f你通过“非法”渠道偷窃了一个SCP-268\n副手持SCP-268时其他人会将你忽略\n利用好这个道具，这会让逃离轻而易举\n躲避安保人员的搜捕\n注意突破收容的异常\n他们会杀死你\n尽一切可能逃离设施",
    tag: "classd_fugitive",
    group: "allclassd",
    items: [
        ["bli:scp268", 1],
        ...
        if (LimitClassD !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointClassD[SpawnPointID].x} ${SpawnPointClassD[SpawnPointID].y} ${SpawnPointClassD[SpawnPointID].z}`);
                    const SpawnAllType = [
                        ClassD,
                        ClassD_Thief,
                        ClassD_Rioters,
                    ...
                    for (let slot = SpawnType.inventory[0]; slot < SpawnType.inventory[1]; slot++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" slot.inventory ${slot} ei:disable 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
                    overworld.runCommand(`replaceitem entity "${PlayerID}" slot.hotbar 8 ei:id_card 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
...
export const Res_Asst = {
    ...
    subtitle: "§b研究助理",
    target_tip: "§f注意突破收容的异常和已经出逃的D级人员\n他们会杀死你\n寻求安全官员与MTF的帮助\n尽一切可能逃离设施\n§e听从研究主管的调度\n他有与你相比更高的权限",
    tag: "res_asst",
    group: "sci",
    items: [["bli:lv2", 1]],
    armor: [
        ...
        if (LimitRes !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    // world.sendMessage(`SpawnPointID:${SpawnPointID} Player:${PlayerID}`);
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointRes[SpawnPointID].x} ${SpawnPointRes[SpawnPointID].y} ${SpawnPointRes[SpawnPointID].z}`);
...
export const Site_Director = {
    ...
    subtitle: "§b站点主管",
    target_tip: "§f组织站点的所有基金会成员遏制收容失效\n无效化你见到的所有SCP项目\n杀死你看到的所有D级人员\n营救仍困在设施内的工作人员\n消灭所有入侵者\n不惜一切代价保护设施",
    tag: "guard_sd",
    group: "allguard",
    items: [
        ["bli:taser_m26", 1],
        ...
        if (LimitSNE !== 0) {
            ...
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    // world.sendMessage(`SpawnPointID:${SpawnPointID} Player:${PlayerID}`);
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSNE[SpawnPointID].x} ${SpawnPointSNE[SpawnPointID].y} ${SpawnPointSNE[SpawnPointID].z}`);
        ...
        if (LimitGOC !== 0) {
            ...
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    // world.sendMessage(`SpawnPointID:${SpawnPointID} Player:${PlayerID}`);
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointGOC[SpawnPointID].x} ${SpawnPointGOC[SpawnPointID].y} ${SpawnPointGOC[SpawnPointID].z}`);
                    let SpawnType;
                    switch (LimitGOC) {
                        case 3:
        ...
        if (LimitSerpentS_Hand !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSH[SpawnPointID].x} ${SpawnPointSH[SpawnPointID].y} ${SpawnPointSH[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
                    ...
                    // }
                    for (let i = 0; i < SpawnType.armor.length; i++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" ${SpawnType.armor[i][0]} 0 ${SpawnType.armor[i][1]} 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
                    overworld.runCommand(`replaceitem entity "${PlayerID}" slot.hotbar 8 ei:id_card 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    for (let i = 0; i < SpawnType.items.length; i++) {
                        overworld.runCommand(`give "${PlayerID}" ${SpawnType.items[i][0]} ${SpawnType.items[i][1]}`);
        ...
        if (Limit173 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP173[SpawnPointID].x} ${SpawnPointSCP173[SpawnPointID].y} ${SpawnPointSCP173[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
                    ...
                    for (let slot = SpawnType.hotbar[0]; slot < SpawnType.hotbar[1]; slot++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" slot.hotbar ${slot} ei:disable 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
                    for (let i = 0; i < SpawnType.items.length; i++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" slot.hotbar ${i} ${SpawnType.items[i][0]} ${SpawnType.items[i][1]} 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
                    for (let j = SpawnType.items.length; j < 9 - SpawnType.items.length; j++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" slot.hotbar ${j} ei:disable 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
                    for (let i = 0; i < SpawnType.armor.length; i++) {
                        overworld.runCommand(`replaceitem entity "${PlayerID}" ${SpawnType.armor[i][0]} 0 ${SpawnType.armor[i][1]} 1 0 {"minecraft:item_lock":{"mode":"lock_in_slot"},"minecraft:keep_on_death":{}}`);
                    }
        ...
        if (Limit457 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP457[SpawnPointID].x} ${SpawnPointSCP457[SpawnPointID].y} ${SpawnPointSCP457[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit066 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP066[SpawnPointID].x} ${SpawnPointSCP066[SpawnPointID].y} ${SpawnPointSCP066[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit049 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP049[SpawnPointID].x} ${SpawnPointSCP049[SpawnPointID].y} ${SpawnPointSCP049[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
...
export function spawn049_2(player) {
    try {
        ...
        if (Limit049_2 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                const SpawnPointID = 0;
                overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP049_2[SpawnPointID].x} ${SpawnPointSCP049_2[SpawnPointID].y} ${SpawnPointSCP049_2[SpawnPointID].z}`);
                // const SpawnAllType = [];
                // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit966 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP966[SpawnPointID].x} ${SpawnPointSCP966[SpawnPointID].y} ${SpawnPointSCP966[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit106 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP106[SpawnPointID].x} ${SpawnPointSCP106[SpawnPointID].y} ${SpawnPointSCP106[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit096 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`gamemode adventure "${PlayerID}"`);
                    const SpawnPointID = 0;
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP096[SpawnPointID].x} ${SpawnPointSCP096[SpawnPointID].y} ${SpawnPointSCP096[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
        ...
        if (Limit035 !== 0) {
            if (AllClassesTag.some((arg) => PlayerTag.includes(arg))) {
                ...
                if (!player.hasTag("dev")) {
                    ...
                    overworld.runCommand(`tp "${PlayerID}" ${SpawnPointSCP035[SpawnPointID].x} ${SpawnPointSCP035[SpawnPointID].y} ${SpawnPointSCP035[SpawnPointID].z}`);
                    // const SpawnAllType = [];
                    // const TypeRandom = Math.floor(Math.random() * SpawnAllType.length);
...
export function spawn008_1(player, spawnlocation) {
    ClearAllTag(player);
...
export function FacilityDetect() {
    ...
    world.sendMessage(`§e[ANNOUNCE] §bControl to NTF, camera scan complete. Multiple stragglers. Their positions are now being broadcasted to you.`);
...
// 清除所有标签
export function ClearAllTag(player) {
    for (let i = 0; i < AllClassesTag.length; i++) {
        player.removeTag(`${AllClassesTag[i]}`);
    }
}
// SCP随机不重复刷新
export function SpawnRandomSCP(num) {
