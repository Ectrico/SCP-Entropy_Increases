...
import { BreachControl, Button, CPTeleport, EleBoxFuse, EleBoxWire, EmergencyExitDoorSetting, EscapeArea, FireDoor, GateA_Blast_Area, GateA_Blast_Position, HelicopterLandingZone, LeverSwitch, LibTeleport, LibTPTarget, Pocket_Dimension, SCP035SpawnPoint, SCP914Area, Terminal, } from "./maps/ft_site_15";
...
export function LeverReset() {
    ...
    for (let i = 0; i < LeverSwitch.Gate.length; i++) {
        ...
        const LeverID = Lever === null || Lever === void 0 ? void 0 : Lever.typeId;
        if (LeverID === "bl:scp008_switch") {
            const NewGeoState = Lever === null || Lever === void 0 ? void 0 : Lever.permutation.withState("ft:geometry", GeoState);
            Lever === null || Lever === void 0 ? void 0 : Lever.setPermutation(NewGeoState);
        } //
        else {
            const NewGeoState = Lever === null || Lever === void 0 ? void 0 : Lever.permutation.withState("bl:door_geomrtry_states", GeoState);
            Lever === null || Lever === void 0 ? void 0 : Lever.setPermutation(NewGeoState);
            const NewLSState = Lever === null || Lever === void 0 ? void 0 : Lever.permutation.withState("bl:logical_sequence", LSState);
            Lever === null || Lever === void 0 ? void 0 : Lever.setPermutation(NewLSState);
        }
...
export function EleBoxFuseReset() {
    ...
    system.runTimeout(() => {
        ...
        overworld.runCommand(`setblock ${EleBoxLocationA.x} ${EleBoxLocationA.y} ${EleBoxLocationA.z} bl:elecbox_fuse_${RandomEleBoxA[0]} ["minecraft:cardinal_direction"="${RandomEleBoxA[2]}"]`);
        overworld.runCommand(`setblock ${EleBoxLocationB.x} ${EleBoxLocationB.y} ${EleBoxLocationB.z} bl:elecbox_fuse_${RandomEleBoxB[0]} ["minecraft:cardinal_direction"="${RandomEleBoxB[2]}"]`);
        //
        for (let i = 0; i < EleBoxFuse.ContainmentRoom.length; i++) {
            const EleBoxContainmentRoomLocation = EleBoxFuse.ContainmentRoom[i][1];
            overworld.runCommand(`setblock ${EleBoxContainmentRoomLocation.x} ${EleBoxContainmentRoomLocation.y} ${EleBoxContainmentRoomLocation.z} bl:elecbox_fuse_${EleBoxFuse.ContainmentRoom[i][0]} ["minecraft:cardinal_direction"="${EleBoxFuse.ContainmentRoom[i][2]}"]`);
        }
    }, 60);
}
// EleBoxWire重置
export function EleBoxWireReset() {
    ...
    system.runTimeout(() => {
        ...
        for (let i = 0; i < EleBoxWire.EmergencyExit.length; i++) {
            const location = EleBoxWire.EmergencyExit[i][0];
            overworld.runCommand(`setblock ${location.x} ${location.y} ${location.z} bl:elecbox_wire ["minecraft:cardinal_direction"="${EleBoxWire.EmergencyExit[i][1]}"]`);
        }
        // 紧急出口随机线缆箱
        ...
        overworld.runCommand(`setblock ${EleBoxLocation.x} ${EleBoxLocation.y} ${EleBoxLocation.z} bl:elecbox_wire ["minecraft:cardinal_direction"="${RandomEleBox[1]}"]`);
        //
    }, 60);
    //
    //
    //
    // 紧急出口大门重置
    const EEDoorRange = new BlockVolume(EleBoxWire.EmergencyExitDoor[0], EleBoxWire.EmergencyExitDoor[1]).getBlockLocationIterator();
    for (const EEDlocation of EEDoorRange) {
        const EEDoor = overworld.getBlock(EEDlocation);
        EEDoor === null || EEDoor === void 0 ? void 0 : EEDoor.setPermutation(EEDoor.permutation.withState("bl:door_geomrtry_states", 0));
        EEDoor === null || EEDoor === void 0 ? void 0 : EEDoor.setPermutation(EEDoor.permutation.withState("bl:logical_sequence", "forward"));
    }
}
// 紧急出口大门状态设置
export function EmergencyExitDoorSet(type) {
...
export function EEOpenSpawnGOC() {
    ...
    const DetectState = DetectBlock === null || DetectBlock === void 0 ? void 0 : DetectBlock.permutation.getState("bl:door_geomrtry_states");
    if (DetectState === 1) {
        support_goc();
    }
}
                ...
                for (const playertag of playerTags) {
                    if (CanEscapeClassesTag.includes(playertag)) {
                        ...
                        if (RoundTime !== undefined) {
                            ...
                            overworld.runCommand(`title "${playerID}" subtitle §e你在 §l§b${TimeRemaining} §r§e内逃离了设施，干的漂亮！`);
                        }
...
export function HelipadEvacuate() {
    ...
    const OmegaState = OmegaWarheadTerminal === null || OmegaWarheadTerminal === void 0 ? void 0 : OmegaWarheadTerminal.permutation.getState("ft:warhead_state");
...
export function PocketDimensionTeleport() {
    for (let i = 0; i < Pocket_Dimension.TelePoint.length; i++) {
        ...
        if (RandomNum <= 0.4) {
            overworld.runCommand(`tp @e[x=${Pocket_Dimension.TelePoint[i].x},y=${Pocket_Dimension.TelePoint[i].y},z=${Pocket_Dimension.TelePoint[i].z},dx=+0,dy=+1,dz=+0] ${ExitPoint.x} ${ExitPoint.y} ${ExitPoint.z}`);
        }
        //
        else if (RandomNum > 0.4 && RandomNum < 0.7) {
            overworld.runCommand(`tp @e[x=${Pocket_Dimension.TelePoint[i].x},y=${Pocket_Dimension.TelePoint[i].y},z=${Pocket_Dimension.TelePoint[i].z},dx=+0,dy=+1,dz=+0] ${Pocket_Dimension.Pocket.x} ${Pocket_Dimension.Pocket.y} ${Pocket_Dimension.Pocket.z}`);
        }
        //
        else {
            overworld.runCommand(`execute as @a[tag=scp106] at @s run damage @e[x=${Pocket_Dimension.TelePoint[i].x},y=${Pocket_Dimension.TelePoint[i].y},z=${Pocket_Dimension.TelePoint[i].z},dx=+0,dy=+1,dz=+0] 100 entity_attack entity @s`);
        }
    }
}
// CheckPoint传送
...
export function LibraryTeleport() {
    for (const TPoint of LibTeleport) {
        ...
        overworld.runCommand(`tp @a[x=${TPoint.x},y=${TPoint.y},z=${TPoint.z},dx=0,dy=1,dz=0] ${Point.x} ${Point.y} ${Point.z}`);
    }
}
// FireDoor重置
export function FireDoorReset() {
    ...
    for (const DoorLocation of FireDoors.getBlockLocationIterator()) {
        ...
        const NewGeo = Door === null || Door === void 0 ? void 0 : Door.permutation.withState("bl:door_geomrtry_states", 0);
        Door === null || Door === void 0 ? void 0 : Door.setPermutation(NewGeo);
        const NewLS = Door === null || Door === void 0 ? void 0 : Door.permutation.withState("bl:logical_sequence", "forward");
        Door === null || Door === void 0 ? void 0 : Door.setPermutation(NewLS);
...
export function AlphaWarheadReset() {
    ...
    AlphaWarheadTerminal === null || AlphaWarheadTerminal === void 0 ? void 0 : AlphaWarheadTerminal.setPermutation(AlphaWarheadTerminal === null || AlphaWarheadTerminal === void 0 ? void 0 : AlphaWarheadTerminal.permutation.withState("ft:warhead_state", "idle"));
}
// Alpha核弹关闭
export function AlphaWarheadDeactivated() {
    const AlphaWarheadTerminal = overworld.getBlock(Terminal.AlphaWarhead);
    AlphaWarheadTerminal === null || AlphaWarheadTerminal === void 0 ? void 0 : AlphaWarheadTerminal.setPermutation(AlphaWarheadTerminal === null || AlphaWarheadTerminal === void 0 ? void 0 : AlphaWarheadTerminal.permutation.withState("ft:warhead_state", "detonated"));
}
// Omega核弹重置
export function OmegaWarheadReset() {
    const OmegaWarheadTerminal = overworld.getBlock(Terminal.OmegaWarhead);
    OmegaWarheadTerminal === null || OmegaWarheadTerminal === void 0 ? void 0 : OmegaWarheadTerminal.setPermutation(OmegaWarheadTerminal === null || OmegaWarheadTerminal === void 0 ? void 0 : OmegaWarheadTerminal.permutation.withState("ft:warhead_state", "idle"));
}
// Omega核弹关闭
export function OmegaWarheadDeactivated() {
    const OmegaWarheadTerminal = overworld.getBlock(Terminal.OmegaWarhead);
    OmegaWarheadTerminal === null || OmegaWarheadTerminal === void 0 ? void 0 : OmegaWarheadTerminal.setPermutation(OmegaWarheadTerminal === null || OmegaWarheadTerminal === void 0 ? void 0 : OmegaWarheadTerminal.permutation.withState("ft:warhead_state", "detonated"));
}
// Omega核弹远程控制重置
export function OmegaWarheadRemoteReset() {
    overworld.runCommand(`setblock ${LeverSwitch.OmegaWarheadRemote.x} ${LeverSwitch.OmegaWarheadRemote.y} ${LeverSwitch.OmegaWarheadRemote.z} bl:lever_on_off ["minecraft:cardinal_direction"="north"]`);
}
export let GateABlast = 1;
export function GateA_Blast(player) {
        ...
        if (BlastPlayer.includes(player)) {
            ...
            function BlastTimeCut() {
                if (BlastTime > 0) {
                    ...
                    overworld.runCommand(`title @a subtitle §cGate-A爆破系统已启动\n${FormatTimeTick(BlastTime)}\n\n\n\n\n\n\n\n\n\n\n`);
                    system.runTimeout(() => {
                        BlastTimeCut();
                    }, 1);
                    if (BlastTime % 40 === 0) {
                    ...
                    for (const BlockLocation of GateA_Blast_Position) {
                        for (let i = 0; i <= 5; i++) {
                            overworld.runCommand(`particle minecraft:knockback_roar_particle ${BlockLocation.x} ${BlockLocation.y} ${BlockLocation.z}`);
                            overworld.runCommand(`particle minecraft:large_explosion ${BlockLocation.x} ${BlockLocation.y} ${BlockLocation.z}`);
                        }
                        PlaySoundForAllPlayer("NUKE2");
                        const DoorBlock = overworld.getBlock(BlockLocation);
                        const NewState = DoorBlock === null || DoorBlock === void 0 ? void 0 : DoorBlock.permutation.withState("bl:door_geomrtry_states", 14);
                        DoorBlock === null || DoorBlock === void 0 ? void 0 : DoorBlock.setPermutation(NewState);
                    }
                }
            }
            if (GateABlast === 1) {
                ...
                for (const Buttons of Button) {
                    ...
                    overworld.runCommand(`setblock ${Position.x} ${Position.y} ${Position.z} bl:on_off_error ["minecraft:cardinal_direction"="${Buttons[1]}"]`);
                }
            }
            break;
        }
...
export function GateA_Reset() {
    ...
    for (const BlockLocation of GateA_Blast_Position) {
        for (let i = 0; i <= 5; i++) {
            const DoorBlock = overworld.getBlock(BlockLocation);
            const NewState = DoorBlock === null || DoorBlock === void 0 ? void 0 : DoorBlock.permutation.withState("bl:door_geomrtry_states", 0);
            DoorBlock === null || DoorBlock === void 0 ? void 0 : DoorBlock.setPermutation(NewState);
        }
    }
    // 按钮重置
    for (const Buttons of Button) {
        const Position = Buttons[0];
        const Button = overworld.getBlock(Position);
        overworld.runCommand(`setblock ${Position.x} ${Position.y} ${Position.z} bl:on_off_card5_w_check ["minecraft:cardinal_direction"="${Buttons[1]}"]`);
    }
}
// SCP-914清理
...
export function SCP035Spawn() {
    ...
    overworld.spawnEntity("ei:scp035_entity", { x: SpawnPoint.x + 0.5, y: SpawnPoint.y, z: SpawnPoint.z + 0.5 });
}
