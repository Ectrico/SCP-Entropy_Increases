...
import { AK103_SkinList, HK_MP7_SkinList, M4A1_S_SkinList, M4A1_SkinList } from "./weapon_skin_data";
// 定义主世界维度
let overworld;
// 定义下界维度
let nether;
...
// 自定义菜单
export function ShowCustomizeMenu(player) {
    // HKMP7皮肤菜单
    const HKMP7SkinMenu = new ActionFormData();
    HKMP7SkinMenu.title(`§d[HK-MP7枪械皮肤]自定义个性化菜单`);
    for (const ThisSkin of HK_MP7_SkinList) {
        try {
            HKMP7SkinMenu.button(`${GetSkinState(player, ThisSkin.id)}${ThisSkin.title}${ThisSkin.level}\n§f许可: ${ThisSkin.permission}\t§f定制所属: ${ThisSkin.ascription}`, `${ThisSkin.icon_path}`);
        }
        catch (_a) { }
    }
    // M4A1-S皮肤菜单
    ...
    for (const ThisSkin of M4A1_S_SkinList) {
        try {
            M4A1_SSkinMenu.button(`${GetSkinState(player, ThisSkin.id)}${ThisSkin.title}${ThisSkin.level}\n§f许可: ${ThisSkin.permission}\t§f定制所属: ${ThisSkin.ascription}`, `${ThisSkin.icon_path}`);
        }
        catch (_b) { }
    }
    // M4A1皮肤菜单
    ...
    for (const ThisSkin of M4A1_SkinList) {
        try {
            M4A1SkinMenu.button(`${GetSkinState(player, ThisSkin.id)}${ThisSkin.title}${ThisSkin.level}\n§f许可: ${ThisSkin.permission}\t§f定制所属: ${ThisSkin.ascription}`, `${ThisSkin.icon_path}`);
        }
        catch (_c) { }
    }
    // AK103皮肤菜单
    ...
    for (const ThisSkin of AK103_SkinList) {
        try {
            AK103SkinMenu.button(`${GetSkinState(player, ThisSkin.id)}${ThisSkin.title}${ThisSkin.level}\n§f许可: ${ThisSkin.permission}\t§f定制所属: ${ThisSkin.ascription}`, `${ThisSkin.icon_path}`);
        }
        catch (_d) { }
    }
    // 主菜单
...
function ReplaceSkinItem(player, skinID, replaceItem) {
    var _a, _b, _c;
    for (let i = 0; i < 9; i++) {
        const Item = (_c = (_b = (_a = player.getComponent("inventory")) === null || _a === void 0 ? void 0 : _a.container) === null || _b === void 0 ? void 0 : _b.getItem(i)) === null || _c === void 0 ? void 0 : _c.typeId;
        if (Item === replaceItem) {
            overworld.runCommand(`replaceitem entity "${player.name}" slot.hotbar ${i} bli:${skinID} 1 0 {"minecraft:keep_on_death":{}}`);
            break;
        }
    }
}
//# sourceMappingURL=custom_skin.js.map